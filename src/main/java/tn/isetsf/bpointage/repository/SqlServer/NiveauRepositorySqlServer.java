...
public interface NiveauRepositorySqlServer extends JpaRepository<NiveauModelSqlServer,Integer> {
    @Query("select new tn.isetsf.bpointage.model.SqlServer.NiveauModelSqlServer(n.COD_NIVEAU,n.Nom_niveau) from NiveauModelSqlServer n where n.COD_NIVEAU in (select s.niveau.COD_NIVEAU from SaisieModelSqlServer s where s.ensiegnant.COD_Enseig=:id)")
    List<NiveauModelSqlServer> findNiveauByEnsieg(int id);
