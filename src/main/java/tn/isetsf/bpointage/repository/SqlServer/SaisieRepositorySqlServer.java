...
public interface SaisieRepositorySqlServer extends JpaRepository<SaisieModelSqlServer,Integer> {
    ...
    @Query("select s.seance.COD_senace from SaisieModelSqlServer s where s.jour.cod_Jour=:idJour and s.niveau.COD_NIVEAU =:idGroup")
    List<Integer> getIdSeanceGroupe(int idJour, int idGroup);
    @Query("select s.seance.COD_senace from SaisieModelSqlServer s where s.jour.cod_Jour=:idJour and s.ensiegnant.COD_Enseig=:idEnseignant")
    List<Integer> getIdSeanceEnseignant(int idJour, int idEnseignant);
    @Query("select s.seance.duree from  SaisieModelSqlServer  s where s.num=:idSeanceAbsence")
    Double getDuree(int idSeanceAbsence);
    @Query("select new tn.isetsf.bpointage.model.MySql.SalleModel(c.COD_salle,c.nom_salle)  from SalleModelSqlServer c where c.COD_salle not in (select s.salle.COD_salle from SaisieModelSqlServer s where s.jour.cod_Jour=:idJour and s.seance.COD_senace in :idSeance) and c.COD_salle not in :idSallePre and c.COD_salle not in :idSalleRatt order by c.nom_salle")
    List<SalleModel> getFreeSallePre(int idJour, List<Integer> idSeance, List<Integer> idSallePre, List<Integer> idSalleRatt);
}
