...
public interface SeanceRepository extends JpaRepository<SeanceModel,Integer> {
    ...
    @Query("select s.id from SeanceModel s where s.debutSeance<= (select ss.debutSeance from SeanceModel ss where ss.id=:idSeance) and s.finSeance>= (select sss.debutSeance from SeanceModel sss where sss.id=:idSeance) ")
