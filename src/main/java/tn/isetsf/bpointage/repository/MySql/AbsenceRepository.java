...
public interface AbsenceRepository extends JpaRepository<AbsenceModel,Integer> {
    @Query("select a from AbsenceModel a where a.idEnsiegnant=:idEnseignant and a.verifier=false and a.idAbsence not in :listIdPre")
    List<AbsenceModel> getAbcensesNonVerifier(int idEnseignant,List<Integer>listIdPre);
    ...
    @Query("select count(a) from AbsenceModel a where a.idEnsiegnant in :listIdEnsei and a.verifier=false")
    ...
    @Query("select count(a) from AbsenceModel a where a.dateAbsence=:date and a.idEnsiegnant in :listIdEnsei and a.verifier=false")
    ...
    @Query("select ab from AbsenceModel ab where ab.annee =:year and ab.semestre =:semestre and ab.idEnsiegnant =:idEnsei  and ab.verifier=false and ab.idAbsence not in :listPre order by ab.dateAbsence desc")
    List<AbsenceModel> getAbcenseByIdEnseignantt(int year, int semestre, int idEnsei,List<Integer> listPre);
